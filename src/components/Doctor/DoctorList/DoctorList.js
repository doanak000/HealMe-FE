...
const DoctorList = (props) => {
    ...
    const listBusinessTempLocal = JSON.parse(
        localStorage.getItem('listBusinessTemp')
    )
    useEffect(async () => {
        if (filterValue === 1) {
            ...
            localStorage.setItem('listBusinessTemp', JSON.stringify(pharmacy))
        }
    ...
    return (
        <div className="row">
            <Input
                placeholder="Nhập tên bác sĩ/dược sĩ/phòng khám để tìm"
                onChange={handleSearch}
                size="large"
                className="ms-2 mb-2 w-100"
                prefix={<SearchOutlined />}
            />
            {listBusinessTempLocal
